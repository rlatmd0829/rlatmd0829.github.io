...
lazyLoding이 걸려있고 <font color='dodgerblue'>Fetch join</font>을 사용하지 않으면 해당엔티티만 조회를 해오고, 지연 로딩으로 설정되어 있는 다른엔티티는 프록시 객체로 가져옵니다.
...
<font color='dodgerblue'>@EntityGraph</font>를 사용하면 특정 엔티티의 연관 관계를 미리 정의된 방식으로 로딩할 수 있으며, 이를 통해 N+1 문제를 해결할 수 있습니다.
...
<font color='dodgerblue'>@BatchSize</font>를 엔티티 클래스에 적용하여, 해당 엔티티의 인스턴스를 로드할 때 일정 수량(batch size)만큼 한 번에 로드합니다.
