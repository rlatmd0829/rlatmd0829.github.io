...
> **<font color='dodgerblue'>yml에 default_batch_fetch_size 설정으로 전역적으로 BatchSize를 설정할 수 있습니다.</font>**

### batch_size와 default_batch_fetch_size 차이
...
- **jdbc batch_size**는 주로 벌크 작업(Bulk Operations)과 관련이 있습니다. 예를 들어, 한 번에 여러 개의 엔터티를 데이터베이스에 삽입하거나 업데이트할 때 사용됩니다.

- **default_fetch_batch_size**는 주로 조회(select)시 연관된 엔터티를 함께 로딩할 때 사용되는 설정입니다.
