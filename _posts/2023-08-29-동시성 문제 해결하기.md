...
void testConcurrentOrderAndChangePrice() throws InterruptedException {
  ...
  Item item1 = itemRepository.findById(1L).orElseThrow(() -> new RuntimeException("Item not found"));
...
위의 테스트 코드는 100개의 스레드가 1번씩 주문과 가격 변경을 수행하도록 했습니다. 이때, 주문과 가격 변경은 동시에 수행되도록 했습니다.
